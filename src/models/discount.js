...
const createDiscount = (data, callback) => {
  const id = uuidv4();
  db.query(
    'INSERT INTO `discount` (`id`, `name`, `desc`, `createdAt`, `updateAt`, `image`, `amount`) VALUES (?,?,?,?,?,?,?)',
...
const updateDiscount = (data, callback) => {
  db.query(
    'UPDATE `discount` SET `name` =?, `desc` = ?, `updateAt` = ?, `image` = ?, `amount` = ?  WHERE id = ?',
    [data.name, data.desc, new Date(), data.image, data.amount, data.id],
    (err, res) => {
      if (err) {
        callback(err, null);
      } else {
